...
public class RunnerHandler extends AbstractHandler {
  ...
  public void stopNoteExecution(final WebSocketSession conn,
    ...
    LOGGER.info("Отановка выполнения ноута noteId: {},  noteUuid: {}", note.getId(), note.getUuid());
  ...
  public void runAllParagraphs(final WebSocketSession conn, final SockMessage fromMessage) {
    ...
    final AuthenticationInfo authenticationInfo = AuthorizationService.getAuthenticationInfo();
    final Note note = safeLoadNote("noteId", fromMessage, Permission.RUNNER, authenticationInfo, conn);
    LOGGER.info("Выполнение всех параграфов ноута  noteId: {},  noteUuid: {}", note.getId(), note.getUuid());
    List<Paragraph> paragraphsForRun = noteService.getParagraphs(note);

    //filter for run several paragraph
    final String json = fromMessage.getOrDefault("paragraphs", null);
    if (!StringUtils.isEmpty(json)) {
      final JsonArray paragraphs = new JsonParser().parse(json).getAsJsonArray();
      final List<String> paragraphUuids = new ArrayList<>(paragraphs.size());
      for (final JsonElement paragraph : paragraphs) {
        final String uuid = paragraph.getAsJsonObject().get("id").getAsString();
  ...
  public void runParagraph(final WebSocketSession conn, final SockMessage fromMessage) {
    ...
    LOGGER.info("Выполнение параграфа noteId: {}, noteUuid: {}, paragraphId: {}",
